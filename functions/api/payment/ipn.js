...
// ✅ Pesapal 401 token auto-refresh — prevents stuck PENDING payments
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    // ── FIX: pass env so fetchPesapalStatus can refresh token on 401 ──
    const pStatus = await fetchPesapalStatus(OrderTrackingId, token, 3, env);
...
// ── FIXED: added env param so 401 triggers a fresh token fetch ──
async function fetchPesapalStatus(orderTrackingId, token, retries = 3, env) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      // ── 401 = token expired — clear cache and get a fresh one ──
      if (res.status === 401) {
        console.warn(`[IPN] 401 on attempt ${attempt} — clearing cached token and refreshing`);
        try { await env.KV.delete('pesapal_token'); } catch {}
        currentToken = await getPesapalToken(env);
        continue; // retry immediately with new token, don't count as failed attempt
      }
